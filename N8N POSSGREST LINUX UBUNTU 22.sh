...
#------------------------------------------------------------------
# KỊCH BẢN CÀI ĐẶT TỰ ĐỘNG HOÀN CHỈNH (v5 - Dùng Docker Compose v2)
# Tác giả: Ticmiro & Gemini
# Chức năng:
# - (MỚI) Cài đặt Docker Compose v2 (plugin, có dấu cách) để
...
# - Thêm thư mục media công khai (/public).
...
echo -e "${GREEN}Chào mừng đến với kịch bản cài đặt hoàn chỉnh cho n8n! (Phiên bản v5)${NC}"
...
then
    ...
    # Chỉ cài đặt Docker engine, KHÔNG cài 'docker-compose' (cũ)
...
# --- BƯỚC 4: (THAY ĐỔI) CÀI ĐẶT DOCKER COMPOSE v2 (PLUGIN) ---
...
then
    ...
    if [ -z "$LATEST_COMPOSE_VERSION" ]; then
        ...
        LATEST_COMPOSE_VERSION="v2.27.1" # Một phiên bản ổn định dự phòng
    fi
    echo "Phiên bản Docker Compose mới nhất là: $LATEST_COMPOSE_VERSION"

    # Cài đặt vào thư mục plugin CLI
    ...
    then
        ...
        echo "Hãy thử cài đặt thủ công theo hướng dẫn tại: https://docs.docker.com/compose/install/"
        exit 1
    else
...
# (LƯU Ý: tên file vẫn là 'docker-compose.yml', đây là tiêu chuẩn)
...
# --- THAY ĐỔI: Sử dụng 'docker compose' (có dấu cách) ---
# Không cần 'docker pull' thủ công nữa.
...
echo "Bất kỳ file nào bạn đặt trong ${GREEN}${INSTALL_DIR}/public_media${NC} sẽ có thể truy cập tại:"
echo -e "${GREEN}https://{DOMAIN_NAME}/public/tên_file_của_bạn${NC}"
echo ""
